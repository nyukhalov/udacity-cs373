    ...
    while change >= tolerance:
        ...
        for i in range(len(path)):
            ...
            for j in range(len(newpath[i])):
                data_change = weight_data * (path[i][j] - newpath[i][j])
                smooth_change = weight_smooth * (newpath[idx_next][j] + newpath[idx_prev][j] - 2 * newpath[i][j])
                newpath[i][j] += data_change + smooth_change
                change += abs(data_change + smooth_change)
