...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    A_take_indexes = []
    for l in range(dim):
        A_take_indexes.append(2 + l)

    expand_indexes = [0, 1]
    for l in range(2 * num_landmarks):
        expand_indexes.append(4 + l)
    ...
    for i in range(len(data)):
        ...
        # extend the matrixes
        Omega = Omega.expand(dim + 2, dim + 2, expand_indexes, expand_indexes)
        Xi = Xi.expand(dim + 2, 1, expand_indexes, [0])
        ...
        for j in range(len(movement)):
            ...
            # fill motion data 
            idx_prev = j
            Omega.value[idx_prev][idx_prev] += 1.0 / motion_noise
            Omega.value[idx_prev][idx] -= 1.0 / motion_noise
            Omega.value[idx][idx_prev] -= 1.0 / motion_noise
            Omega.value[idx][idx] += 1.0 / motion_noise
            Xi.value[idx_prev][0] -= move / motion_noise
            Xi.value[idx][0] += move / motion_noise
                
            # fill measurement data
            for m in measurement:
                landmark_no = m[0]
                landmark_distance = m[1 + j]
                idx_landmark = (2 * (2 + landmark_no)) + j
                Omega.value[idx_prev][idx_prev] += 1.0 / measurement_noise
                Omega.value[idx_prev][idx_landmark] -= 1.0 / measurement_noise
                Omega.value[idx_landmark][idx_prev] -= 1.0 / measurement_noise
                Omega.value[idx_landmark][idx_landmark] += 1.0 / measurement_noise
                Xi.value[idx_prev][0] -= landmark_distance / measurement_noise
                Xi.value[idx_landmark][0] += landmark_distance / measurement_noise

        # reduce the matrixes
        A = Omega.take([0, 1], A_take_indexes)
        A_t = A.transpose()
        B = Omega.take([0, 1], [0, 1])
        B_inverse = B.inverse()
        C = Xi.take([0, 1], [0])
        D = A_t * B_inverse
        Omega_prime = Omega.take(A_take_indexes, A_take_indexes)
        Xi_prime = Xi.take(A_take_indexes, [0])
        Omega = Omega_prime - (D * A)
        Xi = Xi_prime - (D * C)
