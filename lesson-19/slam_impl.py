...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    # *2 as we keep both x and y
    # +1 for current location
    size = 2 * (num_landmarks + len(data) + 1)
    
    print 'Matrix size: %d' % size
    
    omega = matrix()
    omega.zero(size, size)
    
    xi = matrix()
    xi.zero(size, 1)
    
    # init position
    init = [50.0, 50.0] # initial position
    ...
    for i in range(len(data)):
        # Z - distances to landmarks
        # move - robot`s movement
        Z, move = data[i]
        for j in range(len(move)):
            idx = j + (2 * i)
            idx_next = j + (2 * (i + 1))
            m = move[j] # move on x or y axis
            omega.value[idx][idx] += 1.0 / motion_noise
            omega.value[idx][idx_next] += - (1.0 / motion_noise)
            omega.value[idx_next][idx_next] += 1.0 / motion_noise
            omega.value[idx_next][idx] += - (1.0 / motion_noise)
            xi.value[idx][0] += - (m / motion_noise)
            xi.value[idx_next][0] += m / motion_noise
            ...
            for z in Z:
                ...
                idx_l = j + (2 * (1 + len(data) + l_num))
                
                omega.value[idx][idx] += 1.0 / measurement_noise
                omega.value[idx][idx_l] += - (1.0 / measurement_noise)
                omega.value[idx_l][idx] += - (1.0 / measurement_noise)
                omega.value[idx_l][idx_l] += 1.0 / measurement_noise
                xi.value[idx][0] += - (l_distance / measurement_noise)
                xi.value[idx_l][0] += l_distance / measurement_noise
